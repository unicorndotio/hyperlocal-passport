...
Install core workflow skills into target agents plus any setup assets shipped by
enabled extensions.
...
Execute PRD task files sequentially from a workflow directory through the shared
daemon.
...
Execute a single ad hoc prompt through the ACP runtime. Provide prompt as
argument, via `--prompt-file`, or stdin.
...
Fetch review comments from a provider and write them into
`.compozy/tasks/<name>/reviews-NNN/`.
...
By default, `reviews fetch` imports CodeRabbit review-body comments for
`nitpick`, `minor`, and `major`. Use `[fetch_reviews].nitpicks = false` in
`.compozy/config.toml` to disable that import.
...
Reconcile authored workflow artifacts under `.compozy/tasks/` into the daemon
`global.db` catalog.
...
Move workflows that synced daemon state marks as complete to
...
View detailed agent definition, runtime defaults, and MCP servers for a named
agent.
