...
Use these rules to keep Compozy workflow memory useful across repeated PRD task
runs.
...
Use the shared workflow memory for context that should survive across multiple
...
- large code excerpts
- facts that are already explicit in `_prd.md`, `_techspec.md`, `_tasks.md`, or
  the repo itself
...
- preserve current state, durable decisions, reusable learnings, open risks, and
  handoffs
