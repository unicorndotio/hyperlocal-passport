...
5. `drizzle.config.ts` MUST be created at project root with schema path pointing to `src/db/schema.ts` and output directory `src/db/migrations`.
...
- [ ] Add PostgreSQL service to docker-compose.yml with health check configuration
- [ ] Add Drizzle Gateway service to docker-compose.yml with MASTERPASS and port mapping
- [ ] Update web app service with PG_CONNECTION env var, remove DENO_KV_PATH
- [ ] Configure service dependency ordering (depends_on with health check)
- [ ] Create drizzle.config.ts at project root
- [ ] Create .env.example with PG_CONNECTION and MASTERPASS defaults
- [ ] Verify docker-compose config is valid with `docker compose config`
...
- [ ] `docker compose up -d` starts PostgreSQL, Gateway, and web app
- [ ] `docker compose ps` shows all services as healthy within 30 seconds
- [ ] `curl -s -o /dev/null -w '%{http_code}' http://localhost:4983` returns 200 (Gateway login page)
- [ ] Web app env contains `PG_CONNECTION` (verify via `docker compose exec web env | grep PG_CONNECTION`)
- [ ] PostgreSQL `pg_isready` passes via docker health check
