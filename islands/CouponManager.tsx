    ...
    try {
      const res = await fetch(`/api/businesses/${businessId}/coupons`, {
        ...
        body: JSON.stringify({
          ...
          userMonthlyLimit: userMonthlyLimit ? parseInt(userMonthlyLimit) : null,
      ...
      if (res.ok) {
        ...
        setError(data.error || 'Erro ao criar cupom. Verifique os dados e tente novamente.')
      }
    } catch {
  ...
  return (
    <div className='space-y-6'>
      <div className='flex justify-between items-center'>
        <h3 className='text-lg font-medium text-slate-900'>Cupons Cadastrados</h3>
        <Button 
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className='bg-slate-50 p-6 rounded-lg border border-slate-200 space-y-4'>
          <h4 className='font-semibold text-slate-800 text-sm'>Novo Cupom de Desconto</h4>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div className='space-y-1'>
              <label className='text-[10px] font-bold text-slate-500 uppercase tracking-wider'>Título do Cupom</label>
              <input 
                type="text" 
                value={title}
            ...
            <div className='space-y-1'>
              ...
              <select 
                value={type}
                onChange={(e) => setType((e.target as HTMLSelectElement).value as 'basic' | 'special')}
            ...
            <div className='space-y-1'>
              ...
              <input 
                ...
                onInput={(e) => setDiscountPercent((e.target as HTMLInputElement).value)}
                placeholder="5 - 30"
                min="5"
                max="30"
                className='w-full px-3 py-2 border border-slate-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white'
            ...
            <div className='space-y-1'>
              <label className='text-[10px] font-bold text-slate-500 uppercase tracking-wider'>Validade (Opcional)</label>
              <input 
                type="date" 
                value={validUntil}
                onInput={(e) => setValidUntil((e.target as HTMLInputElement).value)}
                className='w-full px-3 py-2 border border-slate-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white'
              />
            </div>
            <div className='space-y-1'>
              <label className='text-[10px] font-bold text-slate-500 uppercase tracking-wider'>Limite Global (Opcional)</label>
              <input 
                type="number" 
                value={globalLimit}
                onInput={(e) => setGlobalLimit((e.target as HTMLInputElement).value)}
                placeholder="Ex: 50 (total de resgates)"
                className='w-full px-3 py-2 border border-slate-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white'
              />
            </div>
            <div className='space-y-1'>
              <label className='text-[10px] font-bold text-slate-500 uppercase tracking-wider'>Limite por Usuário (Mensal)</label>
              <input 
                type="number" 
                value={userMonthlyLimit}
                onInput={(e) => setUserMonthlyLimit((e.target as HTMLInputElement).value)}
                placeholder="Ex: 1"
                className='w-full px-3 py-2 border border-slate-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white'
              />
            </div>
          ...
          <div className='space-y-1'>
            <label className='text-[10px] font-bold text-slate-500 uppercase tracking-wider'>Descrição (Opcional)</label>
            <textarea 
              value={description}
            ...
            {coupons.length === 0 ? (
              ...
              coupons.map((coupon) => (
                <tr key={coupon.id} className='hover:bg-slate-50 transition-colors'>
                  ...
                  <td className='px-6 py-4'>
                    <span className='font-semibold text-blue-600'>{coupon.discountPercent}%</span>
                  </td>
                  <td className='px-6 py-4'>
                    <div className='flex flex-col gap-1.5 w-32'>
                      <div className='flex justify-between text-[10px] font-medium text-slate-600'>
                        <span>Resgatados</span>
                        <span>{coupon.globalClaimedCount} / {coupon.globalLimit || '∞'}</span>
                      </div>
                      {coupon.globalLimit ? (
                        <div className='w-full h-1.5 bg-slate-200 rounded-full overflow-hidden'>
                          <div 
                            className='h-full bg-blue-500 transition-all duration-500' 
                            style={{ width: `${Math.min(100, (coupon.globalClaimedCount / coupon.globalLimit) * 100)}%` }}
                          />
                        </div>
                      ) : (
                        <div className='w-full h-1.5 bg-slate-200 rounded-full overflow-hidden'>
