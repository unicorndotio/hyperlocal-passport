...
export default function CheckoutCalculator({ businessId }: CheckoutCalculatorProps) {
  const code = useSignal('')
  const amountStr = useSignal('') // For display, e.g., "R$ 0,00"
  const amountCents = useSignal(0)
  const loading = useSignal(false)
  const result = useSignal<{ success: boolean; data?: any; error?: string } | null>(null)
  ...
  const toggleScanner = async () => {
    ...
    if (scannerVisible.value) {
      try {
        ...
        result.value = { success: false, error: 'Não foi possível carregar o scanner' }
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        result.value = { success: false, error: errorText || 'Erro ao validar código' }
      }
    } catch (err) {
  ...
  if (result.value?.success) {
    ...
    return (
      <div class="flex flex-col items-center justify-center py-8 animate-in fade-in zoom-in duration-300">
        ...
        <h2 class="text-2xl font-bold text-slate-900 mb-2">Validado com Sucesso!</h2>
          ...
          <div class="relative">
            ...
            <button
              ...
              class={cn(
                ...
                scannerVisible.value ? "bg-red-100 text-red-600" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
              )}
            >
