...
export default function ApprovalDashboard() {
  ...
  const [selectedDocs, setSelectedDocs] = useState<{ idPhoto: string; residenceProof: string } | null>(null)
  ...
  async function handleAction(userId: string, status: 'approved' | 'rejected') {
    if (!confirm(`Tem certeza que deseja ${status === 'approved' ? 'aprovar' : 'rejeitar'} este morador?`)) {
      return
    }
  ...
  if (loading) return <div className="p-8 text-center text-slate-500">Carregando moradores pendentes...</div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 font-medium text-slate-700">Documentos</th>
                <th className="px-4 py-3 font-medium text-slate-700 text-right">Ações</th>
              </tr>
            </thead>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6 relative shadow-2xl">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <p className="text-sm font-medium text-slate-500 mb-2 uppercase tracking-wider">Documento de Identidade (RG/CNH)</p>
                <div className="border rounded-lg overflow-hidden bg-slate-100 min-h-[300px] flex items-center justify-center">
                  {selectedDocs.idPhoto.toLowerCase().endsWith('.pdf') ? (
                     <iframe src={selectedDocs.idPhoto} className="w-full h-[400px]" />
                  ) : (
                    <img src={selectedDocs.idPhoto} alt="RG/CNH" className="max-w-full h-auto" />
                  )}
                </div>
                <a href={selectedDocs.idPhoto} target="_blank" className="text-sm text-blue-600 mt-2 block hover:underline">Abrir em nova aba</a>
              </div>
              
              <div>
                <p className="text-sm font-medium text-slate-500 mb-2 uppercase tracking-wider">Comprovante de Residência</p>
                <div className="border rounded-lg overflow-hidden bg-slate-100 min-h-[300px] flex items-center justify-center">
                  {selectedDocs.residenceProof.toLowerCase().endsWith('.pdf') ? (
                     <iframe src={selectedDocs.residenceProof} className="w-full h-[400px]" />
                  ) : (
                    <img src={selectedDocs.residenceProof} alt="Comprovante" className="max-w-full h-auto" />
                  )}
                </div>
                <a href={selectedDocs.residenceProof} target="_blank" className="text-sm text-blue-600 mt-2 block hover:underline">Abrir em nova aba</a>
              </div>
            </div>
