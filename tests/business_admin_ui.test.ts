...
Deno.test({
  name: 'Business API Endpoints Integration',
  sanitizeOps: false,
  ...
  fn: async (t) => {
    ...
    try {
      ...
      await t.step(
        'POST /api/businesses rejects missing category',
        async () => {
          const formData = new FormData()
          formData.append('name', 'Test')
          formData.append('cnpj', '12345678000195')
      ...
      await t.step(
        ...
        async () => {
          ...
          const [dbUser] = await db
            ...
            .limit(1)
          assertEquals(dbUser.role, 'business')
        },
      )
      ...
      await t.step(
        'PUT /api/businesses/:id updates business data',
        async () => {
          ...
          assertEquals(
            updated.description,
            'Hambúrgueres artesanais atualizados',
          )
      ...
      await db.delete(schema.businesses).where(
        eq(schema.businesses.id, createdBusinessId),
      )
