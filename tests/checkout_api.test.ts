...
Deno.test("Checkout Validation API", async (t) => {
  ...
  const getSessionStub = stub(auth.api, "getSession", () => {
    return Promise.resolve({
      ...
      session: { id: "sess_1", userId: currentUserId, expiresAt: new Date(Date.now() + 3600000) },
    } as any);
  });
  ...
  try {
    await t.step("POST /api/transactions/validate - Success", async () => {
      ...
      const req = new Request("http://localhost:8000/api/transactions/validate", {
        method: "POST",
      ...
      const kvTransaction = await kv.get<Transaction>(["transactions", transaction.id]);
    ...
    await t.step("POST /api/transactions/validate - Wrong Business", async () => {
      ...
      await kv.set(["businesses", otherBizId], { ...business, id: otherBizId, userId: otherUserId });
      
      const newCode = "OTHER1";
      await kv.set(["redemptions", newCode], { ...redemption, id: newCode, businessId: otherBizId });
    ...
    await t.step("POST /api/transactions/validate - Expired Coupon", async () => {
      const expCode = "EXPIRED_" + Math.random().toString(36).slice(2);
      const expCouponId = "coupon_exp_" + Math.random().toString(36).slice(2);
      await kv.set(["coupons", expCouponId], { ...coupon, id: expCouponId, validUntil: Date.now() - 10000 });
      await kv.set(["redemptions", expCode], { ...redemption, id: expCode, couponId: expCouponId });
    ...
    await t.step("POST /api/transactions/validate - Unauthorized Role", async () => {
    ...
    await t.step("POST /api/transactions/validate - Missing body fields", async () => {
      ...
      const req2 = new Request("http://localhost:8000/api/transactions/validate", {
        method: "POST",
        body: JSON.stringify({ code: "ABC" }), // missing amountCents
      });
      const res2 = await (validateHandler as any).POST({ req: req2 });
      assertEquals(res2.status, 400);
      assertEquals(await res2.text(), "Invalid amountCents: must be a positive number");
    });
    ...
    await t.step("POST /api/transactions/validate - Redemption Not Found", async () => {
      const req = new Request("http://localhost:8000/api/transactions/validate", {
        method: "POST",
    ...
    await t.step("POST /api/transactions/validate - Business Not Found", async () => {
      ...
      const res = await (validateHandler as any).POST({ req });
      assertEquals(res.status, 404);
      assertEquals(await res.text(), "Business profile not found for this user");
      currentUserId = businessUserId;
    });

    await t.step("POST /api/transactions/validate - Coupon Inactive", async () => {
      const inactCode = "INACTIVE";
      const inactCouponId = "coupon_inact";
      await kv.set(["coupons", inactCouponId], { ...coupon, id: inactCouponId, isActive: false });
      await kv.set(["redemptions", inactCode], { ...redemption, id: inactCode, couponId: inactCouponId });
