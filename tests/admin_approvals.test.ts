...
async function setupTestUser(userId: string, status: 'pending' | 'approved' | 'rejected' = 'pending') {
  const user = {
    id: userId,
    name: 'Test User',
    cpf: '12345678901',
  ...
  if (status === 'pending') {
    await kv.set(['approvals', 'pending', userId], { userId, createdAt: user.createdAt })
  }
  return user
}
...
Deno.test('Admin Approvals API', async (t) => {
  await t.step('GET /api/admin/approvals/pending returns pending users', async () => {
  ...
  await t.step('POST /api/admin/approvals/:userId approves a user', async () => {
    const userId = crypto.randomUUID()
    await setupTestUser(userId)

    const req = new Request(`http://localhost:8000/api/admin/approvals/${userId}`, {
      method: 'POST',
  ...
  await t.step('POST /api/admin/approvals/:userId returns 400 for invalid status', async () => {
    const userId = crypto.randomUUID()
    await setupTestUser(userId)
  ...
  await t.step('POST /api/admin/approvals/:userId returns 400 for invalid JSON', async () => {
    const userId = crypto.randomUUID()
    const req = new Request(`http://localhost:8000/api/admin/approvals/${userId}`, {
      method: 'POST',
      body: 'not-json',
  ...
  await t.step('POST /api/admin/approvals/:userId returns 404 for missing user', async () => {
    const userId = crypto.randomUUID()
    const req = new Request(`http://localhost:8000/api/admin/approvals/${userId}`, {
      method: 'POST',
      body: JSON.stringify({ status: 'approved' }),
...
Deno.test('Admin Middleware', async (t) => {
  ...
  await t.step('blocks non-admin from /admin*', async () => {
    // Mock resident
    (auth.api as any).getSession = () => Promise.resolve({
  ...
  await t.step('blocks non-admin from /api/admin/*', async () => {
    ...
    (auth.api as any).getSession = () => Promise.resolve({
      session: { id: 's1', userId: 'u1', expiresAt: new Date(Date.now() + 100000), token: 't1', createdAt: new Date(), updatedAt: new Date() },
      user: { id: 'u1', email: 'user@test.com', emailVerified: true, name: 'User', role: 'resident', createdAt: new Date(), updatedAt: new Date() }
    } as any)
  ...
  await t.step('allows admin to /api/admin/*', async () => {
    ...
    (auth.api as any).getSession = () => Promise.resolve({
      session: { id: 's2', userId: 'u2', expiresAt: new Date(Date.now() + 100000), token: 't2', createdAt: new Date(), updatedAt: new Date() },
      user: { id: 'u2', email: 'admin@test.com', emailVerified: true, name: 'Admin', role: 'admin', createdAt: new Date(), updatedAt: new Date() }
    } as any)
