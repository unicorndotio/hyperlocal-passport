...
Deno.test('Admin Middleware', async (t) => {
  ...
  await t.step('blocks POST /api/businesses as resident', async () => {
    ;(auth.api as unknown as { getSession: unknown }).getSession = () =>
      Promise.resolve({
        session: { id: 's3', userId: 'u3', expiresAt: new Date(Date.now() + 100000), token: 't3', createdAt: new Date(), updatedAt: new Date() },
        user: { id: 'u3', email: 'res@test.com', emailVerified: true, name: 'Res', role: 'resident', createdAt: new Date(), updatedAt: new Date() },
      } as unknown)

    const req = new Request('http://localhost:8000/api/businesses', { method: 'POST' })
    const res = await applyMiddleware(req, () => Promise.resolve(new Response('OK')))
    assertEquals(res.status, 403)
    const body = await res.json()
    assertEquals(body.error, 'Forbidden: Admin access required')
  })
  ...
  await t.step('allows GET /api/businesses as resident', async () => {
    ;(auth.api as unknown as { getSession: unknown }).getSession = () =>
      Promise.resolve({
        session: { id: 's4', userId: 'u4', expiresAt: new Date(Date.now() + 100000), token: 't4', createdAt: new Date(), updatedAt: new Date() },
        user: { id: 'u4', email: 'res2@test.com', emailVerified: true, name: 'Res2', role: 'resident', createdAt: new Date(), updatedAt: new Date() },
      } as unknown)

    const req = new Request('http://localhost:8000/api/businesses', { method: 'GET' })
    const res = await applyMiddleware(req, () => Promise.resolve(new Response('OK')))
    assertEquals(res.status, 200)
  })
  ...
  await t.step('blocks /api/coupons as resident', async () => {
    ;(auth.api as unknown as { getSession: unknown }).getSession = () =>
      Promise.resolve({
        session: { id: 's5', userId: 'u5', expiresAt: new Date(Date.now() + 100000), token: 't5', createdAt: new Date(), updatedAt: new Date() },
        user: { id: 'u5', email: 'res3@test.com', emailVerified: true, name: 'Res3', role: 'resident', createdAt: new Date(), updatedAt: new Date() },
      } as unknown)
  ...
  await t.step('allows /api/coupons as business', async () => {
    ;(auth.api as unknown as { getSession: unknown }).getSession = () =>
      Promise.resolve({
        session: { id: 's6', userId: 'u6', expiresAt: new Date(Date.now() + 100000), token: 't6', createdAt: new Date(), updatedAt: new Date() },
        user: { id: 'u6', email: 'biz@test.com', emailVerified: true, name: 'Biz', role: 'business', createdAt: new Date(), updatedAt: new Date() },
      } as unknown)
  ...
  await t.step('allows /api/coupons as admin', async () => {
    ;(auth.api as unknown as { getSession: unknown }).getSession = () =>
      Promise.resolve({
        session: { id: 's7', userId: 'u7', expiresAt: new Date(Date.now() + 100000), token: 't7', createdAt: new Date(), updatedAt: new Date() },
        user: { id: 'u7', email: 'admin2@test.com', emailVerified: true, name: 'Admin2', role: 'admin', createdAt: new Date(), updatedAt: new Date() },
      } as unknown)
  ...
  await t.step('blocks resident from /business page', async () => {
    ;(auth.api as unknown as { getSession: unknown }).getSession = () =>
      Promise.resolve({
        session: { id: 's8', userId: 'u8', expiresAt: new Date(Date.now() + 100000), token: 't8', createdAt: new Date(), updatedAt: new Date() },
        user: { id: 'u8', email: 'resident@test.com', emailVerified: true, name: 'Resident', role: 'resident', createdAt: new Date(), updatedAt: new Date() },
      } as unknown)
  ...
  await t.step('allows business user to /business page', async () => {
    ;(auth.api as unknown as { getSession: unknown }).getSession = () =>
      Promise.resolve({
        session: { id: 's9', userId: 'u9', expiresAt: new Date(Date.now() + 100000), token: 't9', createdAt: new Date(), updatedAt: new Date() },
        user: { id: 'u9', email: 'biz@test.com', emailVerified: true, name: 'Biz', role: 'business', createdAt: new Date(), updatedAt: new Date() },
      } as unknown)
