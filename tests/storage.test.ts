...
import { deleteFile, optimizeImage, uploadFile } from '../lib/storage.ts'
...
Deno.test(
  ...
  async (t) => {
    ...
    await t.step('uploadFile name generation and validation', async () => {
      ...
      const filename = await uploadFile(mockFile, { skipOptimization: true })
  ...
  async (t) => {
    ...
    await t.step(
      ...
      async () => {
        ...
        const filename = await uploadFile(file, { skipOptimization: true })
...
Deno.test(
  ...
  async (t) => {
    const testDir = await Deno.makeTempDir({
      prefix: 'local_passport_img_opt_',
    })
    const origDir = Deno.env.get('UPLOADS_DIR')
    Deno.env.set('UPLOADS_DIR', testDir)

    await t.step('compresses and resizes JPEG image', async () => {
      let sharp
      try {
        sharp = (await import('npm:sharp@^0.35.2')).default
      } catch {
        return
      }
      ...
      const filename = 'test-2000px.jpg'
      const filePath = join(testDir, filename)
      await Deno.writeFile(filePath, buffer)
      const originalSize = (await Deno.stat(filePath)).size

      await optimizeImage(testDir, filename)

      const compressedSize = (await Deno.stat(filePath)).size
      assert(compressedSize < originalSize, 'JPEG should be compressed')
      ...
      assert(result.width !== undefined, 'Width should be readable')
      assert(result.width! <= 1200, 'Width should be <= 1200px')
    })

    await t.step('compresses PNG image', async () => {
      let sharp
      try {
        sharp = (await import('npm:sharp@^0.35.2')).default
      } catch {
      ...
      const filename = 'test-blue.png'
      const filePath = join(testDir, filename)
      await Deno.writeFile(filePath, buffer)
      const _originalSize = (await Deno.stat(filePath)).size

      await optimizeImage(testDir, filename)

      const compressedSize = (await Deno.stat(filePath)).size
      assert(compressedSize > 0, 'Compressed PNG should not be empty')
      // PNG compression may not always reduce size for small images
      // so just verify the file is still valid
      const meta = await sharp(filePath).metadata()
      assert(meta.format === 'png', 'Should remain a PNG')
    })

    await t.step('rejects oversized image with descriptive error', async () => {
      const bigContent = new Uint8Array(6 * 1024 * 1024)
    ...
    await t.step(
      'non-image file passes through without compression',
      async () => {
        const pdfContent = 'fake-pdf-bytes-here'
        ...
        assertEquals(content, pdfContent, 'PDF content should be unchanged')
      },
    )

    await t.step('image metadata is preserved in database', async () => {
      ...
      const [meta] = await db.select().from(schema.fileMetadata)
        .where(eq(schema.fileMetadata.filename, filename))
        .limit(1)

      assertExists(meta)
      assertEquals(meta.userId, 'test-user-opt')
    ...
    await t.step('small image within size limit passes through', async () => {
      const smallContent = new Uint8Array(1024) // 1KB
      const file = new File([smallContent], 'small.jpg', {
        type: 'image/jpeg',
      })
      const filename = await uploadFile(file, { skipOptimization: true })
      assertExists(filename)
      assertEquals(filename.endsWith('.jpg'), true)
