...
Deno.test("Coupon Redemption API", async (t) => {
  ...
  const getSessionStub = stub(auth.api, "getSession", () => {
    return Promise.resolve({
      ...
      session: { id: "sess_1", userId, expiresAt: new Date(Date.now() + 3600000) },
    } as any);
  });

  try {
    await t.step("POST /api/coupons/:id/redeem - Success", async () => {
      const req = new Request(`http://localhost:8000/api/coupons/${couponId}/redeem`, {
        method: "POST",
      });
    ...
    await t.step("POST /api/coupons/:id/redeem - Global Limit Reached", async () => {
      // Set count to limit
    ...
    await t.step("POST /api/coupons/:id/redeem - Expired", async () => {
      const c = (await kv.get<Coupon>(["coupons", couponId])).value!;
      await kv.set(["coupons", couponId], { ...c, validUntil: Date.now() - 1000 });
    ...
    await t.step("POST /api/coupons/:id/redeem - Inactive", async () => {
      const c = (await kv.get<Coupon>(["coupons", couponId])).value!;
      await kv.set(["coupons", couponId], { ...c, isActive: false, validUntil: undefined });
    ...
    await t.step("POST /api/coupons/:id/redeem - Not Found", async () => {
      const req = new Request(`http://localhost:8000/api/coupons/nonexistent/redeem`, {
        method: "POST",
      });
      const res = await (redeemHandler as any).POST({
        req,
    ...
    await t.step("POST /api/coupons/:id/redeem - User Monthly Limit", async () => {
      const c = (await kv.get<Coupon>(["coupons", couponId])).value!;
      await kv.set(["coupons", couponId], { ...c, isActive: true, userMonthlyLimit: 1, globalClaimedCount: 0 });

      const now = Date.now();
      // Simulate existing redemption
      await kv.set(["user_redemptions", userId, now - 1000], { 
        id: "ALREADY", couponId, userId, status: "active", redeemedAt: now - 1000 
      });
