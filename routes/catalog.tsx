...
export const handler = define.handlers({
  async GET(ctx) {
    ...
    const allCategories = ['Todos', ...new Set(businesses.map(b => b.category))].sort()
  ...
  return (
    <div class="px-4 py-6 max-w-md mx-auto min-h-screen bg-background">
      ...
      <header class="mb-8">
        ...
        <p class="text-muted-foreground text-sm">Descubra benefícios exclusivos no seu bairro.</p>
      </header>
      ...
      <div class="grid gap-6">
        {businesses.length === 0 ? (
          <div class="text-center py-12 bg-card rounded-xl border border-dashed border-border">
            <p class="text-muted-foreground">Nenhuma empresa encontrada nesta categoria.</p>
          </div>
        ) : (
          businesses.map((business: Business) => (
            <a key={business.id} href={`/business/${business.id}`} class="block transition-transform active:scale-98">
              <Card className="hover:ring-1 hover:ring-primary/50 transition-all border-none shadow-sm bg-card">
                <CardHeader className="flex-row items-center gap-4 space-y-0 pb-2">
                  ...
                  <div class="flex-1 min-w-0">
                    <Badge variant="outline" className="mb-1 text-[10px] uppercase tracking-wider font-bold">
                      {business.category}
                    </Badge>
                ...
                <CardContent>
                  <p class="text-muted-foreground line-clamp-2 text-xs leading-relaxed">
                    {business.description || "Confira os benefícios exclusivos disponíveis para moradores nesta loja."}
                  </p>
                </CardContent>
              </Card>
            </a>
